  ...
  return (
    <>
      <div className=" inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto md:ml-6 sm:pr-0">
        <Link href="/" onClick={() => setIsOpen(true)} className="bg-blue-950 rounded-full px-6 py-4 text-lg text-black md:text-white shadow-sm transition-colors duration-75 group gap-[0.25em] inline-flex items-center hover:cursor-pointer hover:bg-opacity-90">
          Cotizar
              ...
              <TransitionChild as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                <DialogPanel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  <div className="py-8 lg:py-8 px-4 mx-auto max-w-screen-md">
                    ...
                    <p className="mb-8 lg:mb-16 mt-8 font-light text-center text-gray-500 dark:text-gray-400 sm:text-md">{"subtitle"}</p>
                    <form className="space-y-8" onSubmit={handleSubmit(onSubmit)}>
                      <div className="flex gap-3">
                        ...
                        <div>
                          ...
                          {errors.lastname && <div className="text-grey text-sm">{"errors.lastname"}</div>}
                        </div>
                      </div>
                      <div>
                        ...
                        {errors.email && <div className="text-grey text-sm">{"errors.email"}</div>}
                      </div>
                      <div className="sm:col-span-2">
                        <label htmlFor="message" className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">
                          {"message"}
                        </label>
                        <textarea id="message" {...register("message", { required: true, minLength: 30 })} className={inputClasses} placeholder={"message_text"}></textarea>
                        {errors.message && <div className="text-grey text-sm">{"errors.message"}</div>}
                      </div>
